...
low_cutoff=5; % band pass filter lower cutoff frequency (Hz)
upper_cutoff=15; % band pass filter upper cutoff frequency (Hz)

epoch_start=-0.2; % latency before the event for epoch extraction
epoch_end=0.8; % latency after the event for epoch extraction

max_amplitude=30; % Defines the maximum amplitude allowed in an epoch

smooth=10;
% [ALLEEG EEG CURRENTSET ALLCOM] = eeglab;
EEG = pop_loadset('filename','200p300PzearOzeardrlfpz_3.set','filepath','/arquivos/tcc/Results/28october2014');
...
figure; pop_erpimage(EEG,1, [channel],[],'Standard ERP average',smooth,1,{ 'std'},[],'latency' ,'yerplabel','\muV','erp','on','cbar','on','topo', { [1] EEG.chanlocs EEG.chaninfo } );
[ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3,'retrieve',4,'study',0); 
EEG = eeg_checkset( EEG );

figure; pop_erpimage(EEG,1, [channel],[],'Odd ERP average',smooth,1,{ 'odd'},[],'latency' ,'yerplabel','\muV','erp','on','cbar','on','topo', { [1] EEG.chanlocs EEG.chaninfo } );