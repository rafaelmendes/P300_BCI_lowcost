function [Accuracy, Selectivity, Sensitivity, Specificity, W] = calibration(ALLEEG, EEG, low_cutoff, upper_cutoff, epoch_start,...
...
EEG_std = pop_rmbase( EEG_std, [epoch_start epoch_end]);

EEG_odd = pop_epoch( EEG, {odd}, [epoch_start epoch_end], 'newname', 'epochs', 'epochinfo', 'yes');
EEG_odd = pop_rmbase( EEG_odd, [epoch_start epoch_end]);

% Bad Epochs rejection:
[EEG_std, indexes] = pop_eegthresh(EEG_std,1, channel,-max_amplitude, max_amplitude, epoch_start, epoch_end, 0, 0);
EEG_std = pop_rejepoch( EEG_std, indexes, 0);

[EEG_odd, indexes]  = pop_eegthresh(EEG_odd,1, channel,-max_amplitude, max_amplitude, epoch_start, epoch_end, 0, 0);
